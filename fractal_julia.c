...
/*   Updated: 2018/03/02 20:37:46 by vferreir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int magie_noir(t_map *map, double x, double y)
{
	int i = 0;
	double ima_r = x;
	double ima_i = y;
	double tmp_r = 0;
	double c_r = map->julia_cr;
	double c_i = 0.22;
...
void fractal_julia(t_map *map)
{
	...
	while (++y < SCREEN_HEIGHT)
	{
		...
		{
			i = magie_noir(map, 2.7 / map->zoom * x / SCREEN_WIDTH - 1.35 / map->zoom + map->pos_x,
